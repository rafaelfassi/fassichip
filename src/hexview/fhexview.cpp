...
// Desc: constructor
//------------------------------------------------------------------------------
FHexView::FHexView(QWidget *parent) : QAbstractScrollArea(parent),
                ...
                show_line2_(true), show_line3_(true), show_address_separator_(false) {
...
// Desc:
//------------------------------------------------------------------------------
void FHexView::setShowAddressSeparator(bool value) {
...
// Desc:
//------------------------------------------------------------------------------
QString FHexView::formatAddress(address_t address) {
...
//------------------------------------------------------------------------------
int FHexView::dataSize() const {
        //return data_ ? data_->GetSizeFile(data_->GetSerieMain()) : 0;
    if(!Data) return 0;

        return !Data->IsEmply() ? Data->GetSizeFile(Data->GetSerieMain()) : 0;
...
//------------------------------------------------------------------------------
void FHexView::setFont(const QFont &f) {
...
//------------------------------------------------------------------------------
QMenu *FHexView::createStandardContextMenu() {
...
//------------------------------------------------------------------------------
void FHexView::contextMenuEvent(QContextMenuEvent *event) {
        ...
        if(hasSelectedText()) {
                ...
                while(offset < end) {
                        ...
                        if((offset + chars_per_row) > start) {
                                ...
                                //data_->seek(offset);
                                const QByteArray row_data = ReadData(offset, chars_per_row);//data_->read(chars_per_row);
...
//------------------------------------------------------------------------------
bool FHexView::hasSelectedText() const {
...
//------------------------------------------------------------------------------
bool FHexView::isInViewableArea(int index) const {
...
// Desc:
//------------------------------------------------------------------------------
void FHexView::keyPressEvent(QKeyEvent *event) {
...
//------------------------------------------------------------------------------
int FHexView::hexDumpLeft() const {
...
//------------------------------------------------------------------------------
int FHexView::asciiDumpLeft() const {
...
//------------------------------------------------------------------------------
int FHexView::commentLeft() const {
...
//------------------------------------------------------------------------------
unsigned int FHexView::charsPerWord() const {
...
//------------------------------------------------------------------------------
unsigned int FHexView::addressLen() const {
...
//------------------------------------------------------------------------------
void FHexView::updateScrollbars() {
...
//------------------------------------------------------------------------------
void FHexView::scrollTo(unsigned int offset) {
...
//------------------------------------------------------------------------------
void FHexView::setShowAddress(bool show) {
        show_address_ = show;
        updateScrollbars();
        repaint();
}
...
//------------------------------------------------------------------------------
void FHexView::setShowHexDump(bool show) {
        show_hex_ = show;
        updateScrollbars();
        repaint();
}
...
//------------------------------------------------------------------------------
void FHexView::setShowComments(bool show) {
        show_comments_ = show;
        updateScrollbars();
        repaint();
}
...
//------------------------------------------------------------------------------
void FHexView::setShowAsciiDump(bool show) {
        show_ascii_ = show;
        updateScrollbars();
        repaint();
}
...
//------------------------------------------------------------------------------
void FHexView::setRowWidth(int rowWidth) {
        row_width_ = rowWidth;
        updateScrollbars();
        repaint();
}
...
//------------------------------------------------------------------------------
void FHexView::setWordWidth(int wordWidth) {
...
//------------------------------------------------------------------------------
unsigned int FHexView::bytesPerRow() const {
...
//------------------------------------------------------------------------------
int FHexView::pixelToWord(int x, int y) const {
        int word = -1;
...
//------------------------------------------------------------------------------
void FHexView::mouseDoubleClickEvent(QMouseEvent *event) {
        if(event->button() == Qt::LeftButton) {
                ...
                if(x >= line1() && x < line2()) {
                        ...
                        if(byte_offset <= dataSize()-1)
                        {
                            ...
                            QMessageBox msg;
                            msg.setText(QString::number(selection_start_, 16));
                            msg.exec();
                        }
...
//------------------------------------------------------------------------------
void FHexView::mousePressEvent(QMouseEvent *event) {
        if(event->button() == Qt::LeftButton) {
                const int x = event->x() + horizontalScrollBar()->value() * font_width_;
                const int y = event->y();
                ...
                if(offset < dataSize()) {
                        ...
                        selection_start_ = selection_end_ = dataSize()-1;
                }
                int _PreviousPointer = Pointer;

                //Pointer = selection_start_ <= selection_end_ ? selection_start_ : selection_end_;
                Pointer = selection_start_ < selection_end_ ? selection_end_-1 : selection_end_;
                repaint();
                if(Pointer != _PreviousPointer) emit PointerChange(Pointer);
        }
}
...
//------------------------------------------------------------------------------
void FHexView::mouseMoveEvent(QMouseEvent *event) {
        if(highlighting_ != Highlighting_None) {
                const int x = event->x() + horizontalScrollBar()->value() * font_width_;
                const int y = event->y();

                int offset = pixelToWord(x, y);

                if(offset > dataSize()) offset = dataSize();
...
//------------------------------------------------------------------------------
void FHexView::mouseReleaseEvent(QMouseEvent *event) {
        if(event->button() == Qt::LeftButton) {
...
//------------------------------------------------------------------------------
void FHexView::SetData(FData *_Data) {
...
//------------------------------------------------------------------------------
void FHexView::resizeEvent(QResizeEvent *) {
        updateScrollbars();
}
...
//------------------------------------------------------------------------------
void FHexView::setAddressOffset(address_t offset) {
...
//------------------------------------------------------------------------------
bool FHexView::isSelected(int index) const {
...
//------------------------------------------------------------------------------
void FHexView::drawComments(QPainter &painter, unsigned int offset, unsigned int row, int size) const {
...
//------------------------------------------------------------------------------
void FHexView::drawAsciiDumpToBuffer(QTextStream &stream, unsigned int offset, int size, const QByteArray &row_data) const {
        // i is the byte index
        const int chars_per_row = bytesPerRow();
...
//------------------------------------------------------------------------------
void FHexView::drawCommentsToBuffer(QTextStream &stream, unsigned int offset, int size) const {
        Q_UNUSED(size);
        const address_t address = address_offset_ + offset;
        const QString comment   = comment_server_->comment(address, word_width_);
        stream << comment;
...
//------------------------------------------------------------------------------
QString FHexView::format_bytes(const QByteArray &row_data, int index) const {
        union {
                quint64 q;
                quint32 d;
                quint16 w;
...
//------------------------------------------------------------------------------
void FHexView::drawHexDumpToBuffer(QTextStream &stream, unsigned int offset, int size, const QByteArray &row_data) const {
...
//------------------------------------------------------------------------------
void FHexView::drawHexDump(QPainter &painter, unsigned int offset, unsigned int row, int size, int &word_count, const QByteArray &row_data) const {
...
//------------------------------------------------------------------------------
void FHexView::drawAsciiDump(QPainter &painter, unsigned int offset, unsigned int row, int size, const QByteArray &row_data) const {
        ...
        for(int i = 0; i < chars_per_row; ++i) {
                ...
                if(index < size) {
                        ...
                        //const QString byteBuffer(printable ? QString::fromLatin1(&ch, 1) : &unprintable_char_);
...
//------------------------------------------------------------------------------
void FHexView::paintEvent(QPaintEvent *) {
...
// Name: allBytes() const
//------------------------------------------------------------------------------
QByteArray FHexView::allBytes() const {
        //data_->seek(0);
        //return data_->readAll();
        return ReadData(0, Data->GetSizeFile(Data->GetSerieMain()));
}

//------------------------------------------------------------------------------
// Name: selectedBytes() const
//------------------------------------------------------------------------------
QByteArray FHexView::selectedBytes() const {
        if(hasSelectedText()) {
                const int s = qMin(selection_start_, selection_end_);
                const int e = qMax(selection_start_, selection_end_);

                //data_->seek(s);
                return ReadData(s, e - s);//data_->read(e - s);
        }
...
//------------------------------------------------------------------------------
FHexView::address_t FHexView::selectedBytesAddress() const {
...
//------------------------------------------------------------------------------
unsigned int FHexView::selectedBytesSize() const {
...
//------------------------------------------------------------------------------
FHexView::address_t FHexView::addressOffset() const {
...
//------------------------------------------------------------------------------
void FHexView::setCommentServer(const QSharedPointer<CommentServerInterface> &p) {
        comment_server_ = p;
}
...
//------------------------------------------------------------------------------
QSharedPointer<FHexView::CommentServerInterface> FHexView::commentServer() const {
...
//------------------------------------------------------------------------------
bool FHexView::showHexDump() const {
        return show_hex_;
}
...
//------------------------------------------------------------------------------
bool FHexView::showAddress() const {
...
//------------------------------------------------------------------------------
bool FHexView::showAsciiDump() const {
        return show_ascii_;
}
...
//------------------------------------------------------------------------------
bool FHexView::showComments() const {
...
//------------------------------------------------------------------------------
FHexView::address_t FHexView::firstVisibleAddress() const {
        // current actual offset (in bytes)
        const int chars_per_row = bytesPerRow();
        unsigned int offset = verticalScrollBar()->value() * chars_per_row;
...
QByteArray FHexView::ReadData(int _Offset, int _NBytes) const
{
    QByteArray _Array;
    int _IdFile = Data->GetSerieMain();
    FDataMode DataMode(BITS8, false, false);

    for(int x=_Offset; x < (_Offset + _NBytes); x++)
        _Array.append(Data->GetValue(_IdFile, x, DataMode));

    return _Array;
}

void FHexView::Update()
{
    updateScrollbars();
    repaint();
}

void FHexView::SetSelection(int _Start, int _End)
{
    selection_start_ = _Start;
    selection_end_ = _End;
